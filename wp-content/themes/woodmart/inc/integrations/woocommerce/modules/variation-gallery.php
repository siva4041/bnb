...
if ( ! function_exists( 'woodmart_vg_admin_html' ) ) {
	function woodmart_vg_admin_html( $loop, $variation_data, $variation ) {
		...
		$attachments            = '';
		$variation_gallery_data = get_post_meta( $post->ID, 'woodmart_variation_gallery_data', true );
		...
		echo '<div class="woodmart-variation-gallery-wrapper">';
			...
			echo '<input type="hidden" class="variation-gallery-ids" name="woodmart_variation_gallery[' . esc_attr( $variation->ID ) . ']" value="' . esc_attr( implode( ',', $attachments ) ) . '">';
...
if ( ! function_exists( 'woodmart_quick_view_vg_data' ) ) {
	function woodmart_quick_view_vg_data() {
		if ( ! woodmart_get_opt( 'variation_gallery' ) ) {
			return;
		}

		echo '<script type="text/javascript">';
			echo 'var woodmart_variation_gallery_data = ' . json_encode( woodmart_get_vg_data() );
		echo '</script>';
	}
}
