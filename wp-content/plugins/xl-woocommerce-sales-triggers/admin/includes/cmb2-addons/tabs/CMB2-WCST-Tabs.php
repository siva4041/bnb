...
if ( ! class_exists( 'CMB2_WCST_Tabs', false ) ) {

	/**
	 * Class CMB2_WCST_Tabs
	 *
	 * @since  1.0.0
	 *
	...
	class CMB2_WCST_Tabs {
		...
		public function opening_div( $cmb_id, $object_id, $object_type, $cmb ) {
			...
			// Current cmb2 instance
			CMB2_WCST_Tabs::$cmb = $cmb;

			// Add CMB2_WCST_Tabs custome render callback to instance
			CMB2_Field::$callable_fields[] = 'CMB2_WCST_Tabs_render_row_cb';
		...
		public function render_nav( $cmb_id, $object_id, $object_type, $cmb ) {
			...
			$tabs = $cmb->prop( "wcst_tabs" );

			if ( $tabs ) {

				echo '<ul class="wcst-cmb-tab-nav">';
				$active_nav = true;
		...
		public static function tabs_render_row_cb( $field_args, $field ) {
			...
			if ( 'group' === $field_args['type'] ) {
				...
				if ( $field->args( 'CMB2_WCST_Tabs_render_row_cb' ) ) {
					CMB2_WCST_Tabs::$cmb->peform_param_callback( 'CMB2_WCST_Tabs_render_row_cb' );
				} else {
		...
		public static function tabs_render_group_row_cb( $field_args, $field_group ) {
			...
			if ( $field_group->args( 'cmb2_tabs_render_row_cb' ) ) {
				CMB2_WCST_Tabs::$cmb->render_group_callback( 'cmb2_tabs_render_row_cb' );
			} else {
				CMB2_WCST_Tabs::$cmb->render_group_callback( $field_args, $field_group );
			}
		...
		public function setup_admin_scripts() {
			...
			wp_register_script( 'wcst-cmb-tabs-js', self::url( 'js/tabs.js' ), array( 'jquery' ), self::VERSION );
			wp_enqueue_script( 'wcst-cmb-tabs-js' );

			wp_enqueue_style( 'wcst-cmb2-tabs-style', self::url( 'css/tabs.css' ), array(), self::VERSION );
			wp_enqueue_style( 'wcst-cmb2-tabs-style' );
		...
		public static function url( $path = '' ) {
			...
			/**
			 * Set the variable CMB2_WCST_Tabs_dir
			 */
			$CMB2_WCST_Tabs_dir = trailingslashit( dirname( __FILE__ ) );

			/**
			 * Use CMB2_Utils to gather the url from CMB2_WCST_Tabs_dir
			 */
			$CMB2_WCST_Tabs_url = CMB2_Utils::get_url_from_dir( $CMB2_WCST_Tabs_dir );

			/**
			 * Filter the CMB2 FPSA location url
			 */
			self::$url = trailingslashit( apply_filters( 'CMB2_WCST_Tabs_url', $CMB2_WCST_Tabs_url, self::VERSION ) );
	...
	//Boot the hole thing
	$CMB2_WCST_Tabs = new CMB2_WCST_Tabs();
}
