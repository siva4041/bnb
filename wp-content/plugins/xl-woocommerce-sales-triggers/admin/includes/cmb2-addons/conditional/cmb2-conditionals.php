 ...
 */
if ( ! class_exists( 'WCST_CMB2_Conditionals' ) ) {
	/**
	 * CMB2_Conditionals Plugin.
	 */
	class WCST_CMB2_Conditionals {
		...
		public function admin_footer() {
			...
			if ( is_object( $screen ) && ( $screen->base == 'woocommerce_page_wc-settings' || ( $screen->base == 'post' && ( $screen->post_type == WCST_Common::get_trigger_post_type_slug() || $screen->post_type == 'product' ) ) ) ) {
				...
				wp_enqueue_script( 'wcst-cmb2-conditionals', plugins_url( '/cmb2-conditionals.js', __FILE__ ), array( 'jquery', 'cmb2-scripts' ), self::VERSION, true );
			}
		}
...
//				);
	...
	if ( ( function_exists( 'wp_installing' ) && wp_installing() === false ) || ( ! function_exists( 'wp_installing' ) && ( ! defined( 'WP_INSTALLING' ) || WP_INSTALLING === false ) ) ) {
		add_action( 'plugins_loaded', 'wcst_cmb2_conditionals_init' );
	}

	if ( ! function_exists( 'wcst_cmb2_conditionals_init' ) ) {
		/**
		 * Initialize the class.
		 */
		function wcst_cmb2_conditionals_init() {

			$cmb2_conditionals = new WCST_CMB2_Conditionals();
		}
	}
}