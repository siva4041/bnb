jQuery(document).ready(function ($) {
     ...
     */
    window.WCSTCMB2ConditionalsInit = function (changeContext, conditionContext) {
        ...
        changeContext.on('change', 'input, textarea, select', function (evt) {
            ...
            dependants = WCSTCMB2ConditionalsFindDependants(fieldName, elm, conditionContext);
            ...
            if (dependants.length > 0) {
                ...
                if ('checkbox' === elm.attr('type')) {
                    ...
                    elmValue = WCSTCMB2ConditionalsStringToUnicode(evt.currentTarget.value);
                }
                ;
                ...
                dependants.each(function (i, e) {
                    ...
                    if ('undefined' !== typeof currentFieldName && '' !== currentFieldName && $.inArray(currentFieldName, dependantsSeen) < 0) {
                        ...
                        if (false === shouldShow) {
                            WCSTCMB2ConditionalsRecursivelyHideDependants(currentFieldName, current, conditionContext);
                        }
                        ...
                        else if (current.hasClass("wcst_custom_wrapper_group") || current.hasClass("wcst_custom_wrapper_wysiwyg")) {
                            if ('checkbox' === elm.attr('type')) {
                                if ('undefined' === typeof requiredValue) {
                                    shouldShow = ( checkedValues.length > 0 );
                                } else if ('off' === requiredValue) {
                    ...
                    changeContext.trigger("wcst_conditional_runs", [current, currentFieldName, requiredValue, currentParent, shouldShow, elm, elmValue]);
                });
            }
        });
        ...
        //     if ($(this).hasClass("wcst_custom_wrapper_group")) {
        //         return;
        //     }
        ...
        $('[data-conditional-id]', conditionContext).not(".wcst_custom_wrapper_group").parents('.cmb-row:first').hide({
            "complete": function () {
                $("body").trigger("wcst_trigger_conditional_on_load");
     ...
     */
    function WCSTCMB2ConditionalsFindDependants(fieldName, elm, context) {
     ...
     */
    function WCSTCMB2ConditionalsRecursivelyHideDependants(fieldName, elm, context) {
        var dependants = WCSTCMB2ConditionalsFindDependants(fieldName, elm, context);
        ...
        if (dependants.length > 0) {
            dependants.each(function (i, e) {
                ...
                WCSTCMB2ConditionalsRecursivelyHideDependants(dependantName, dependant, context);
            });
        }
    }


    function WCSTCMB2ConditionalsStringToUnicode(string) {
        var i, result = '',
